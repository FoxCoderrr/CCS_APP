    ...
    <div class="allinfo">
        ...
        <div class="xx">
            ...
            <div class="ll ovh">
                ...
                <span class="fl ml">身份证</span>
            </div>
            <div class="ll ovh">
                <span class="fl">身份证号</span><input class="fl" type="number" v-model="ucard">
            </div>
        </div>
        <div>
        ...
        <p class="tac mt10 zy">注：请上传您的二代身份证，并确保其在有效期内</p>
        <p class="btn" @click="sub">立即绑定</p>
    </div>
</template>
<script>
import $ from "jquery";
export default {
  ...
  mounted() {
    ...
    that
      ...
      .then(function(res) {
        that.path = "http://"+res.data.msg.shiming;
      });
  },
  methods: {
    back() {
    ...
    sub() {
      let that = this;
      if (that.uname != "" && that.ucard != "" && that.data1 != "") {
        that
          .$http({
            url: "/idcardh",
            method: "get",
